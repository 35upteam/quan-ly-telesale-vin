...
# --- 2. LOGIC ĐĂNG NHẬP (KHÔI PHỤC KẾT NỐI) ---
    ...
    try:
        ...
        with tab_tieuchi:
            ...
            if st.button("🚀 Thực hiện lọc", key="btn_filter"):
                ...
                allowed_tầng = LIST_TANG_PHYSICAL[idx_s : idx_e + 1]
                t_df = t_df[t_df['Tầng'].isin(allowed_tầng)]
            ...
            for i, r in res_display.iterrows():
                ...
                if row[6].button("💾", key=f"sv_{i}"):
                    try:
                        ...
                        st.toast(f"Đã lưu ghi chú cho căn {r['Mã đầy đủ']}!", icon="✅")
                    except Exception as e:
                        st.error(f"Lỗi khi lưu: {e}")
                st.markdown("<div class='row-divider'></div>", unsafe_allow_html=True)
    except Exception as e:
        st.error(f"Lỗi hệ thống hiển thị: {e}")
