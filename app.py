...
# --- 1. CẤU HÌNH & CSS (KHÔI PHỤC GIAO DIỆN GỐC) ---
...
st.markdown("""
    ...
    /* Hỗ trợ Mobile lướt ngang bảng thay vì đổi giao diện */
...
@st.cache_data(ttl=600) # Tối ưu tốc độ load trong 10 phút
...
# --- 4. LOGIC ĐĂNG NHẬP (CÓ ADMIN NINH) ---
if not st.session_state['logged_in']:
    ...
    with mid_col:
        ...
        if st.button("Đăng nhập"):
            users_df = load_sheet_data("QUAN_LY_USER")
            auth = users_df[(users_df['Username'] == u_val) & (users_df['Password'] == p_val)]
            if not auth.empty:
    ...
    df_main = load_sheet_data("DATA_CAN_HO")

    # --- 5. FIX LỖI TAB BỘ LỌC ---
    ...
    with tab_tieuchi:
        ...
        with c2: f_s = st.selectbox("Từ tầng", LIST_TANG, index=4)
        with c3: f_e = st.selectbox("Đến tầng", LIST_TANG, index=15)
        ...
        if st.button("🚀 Thực hiện lọc"):
            ...
            if sel_tr:
                t_df['Trục_C'] = t_df['Trục'].apply(lambda x: x.replace(".0", "").zfill(2) if x else "")
                t_df = t_df[t_df['Trục_C'].isin(sel_tr)]
            idx_s, idx_e = LIST_TANG.index(f_s), LIST_TANG.index(f_e)
            allowed = LIST_TANG[idx_s : idx_e + 1]
            t_df = t_df[t_df['Tầng'].isin(allowed)]
            st.session_state['res_df'] = t_df
            st.rerun()

    # --- 6. HIỂN THỊ DANH SÁCH (DẠNG BẢNG GỐC) ---
    ...
    if not res_display.empty:
        ...
        for i, r in res_display.iterrows():
            ...
            if st.session_state.get(s_key):
                ...
                if row[4].button("👁️ Xem", key=f"v_btn_{i}"):
                    st.session_state[s_key] = True
                    st.rerun()
            
            n_val = row[5].text_input("Note", value=r.get('Ghi chú', ''), key=f"in_{i}", label_visibility="collapsed")
            st.markdown('<div class="save-btn">', unsafe_allow_html=True)
            if row[6].button("💾", key=f"sv_{i}"):
                try:
                    sh_data = doc.worksheet("DATA_CAN_HO")
                    cell = sh_data.find(r['Mã đầy đủ'])
                    sh_data.update_cell(cell.row, list(res_display.columns).index('Ghi chú')+1, n_val)
                    st.toast("Đã lưu!")
